...
params:
  ...
  currentInstance: 'Forget20-KP_10_3_1-10_random_10_10'
---
...
plotCones3D(pts[,1:3], rectangle = T, drawPoint = F, argsPolygon3d = list(alpha = 1, color = "lightblue"), drawLines = F)
...
mfrow3d(ceiling(length(ptsLst)/2), 2, sharedMouse = F)
for (s in 1:length(ptsLst)) {
   ...
   for (j in 1:nrow(ptsLst[[s]])) {
      ...
      ids <- plotCones3D(ptsLst[[s]][j, 1:3], rectangle = T, drawPoint = T, argsPolygon3d = list(alpha = 1, color = "blue"), drawLines = T)
      if (any(lst[[i]] == emptyId)) lst[[i]] <- ids else lst[[i]] <- c(lst[[i]], ids)
   }
   names(lst) <- paste0("cone", 1:length(lst))
   lst[[i+1]] <- plotHull3D(ptsLst[[s]][,1:3], addRays = T, argsPolygon3d = list(alpha = 0.25, color = "red"), useRGLBBox = TRUE)$ids
   # res[[s]]$lst <- lst
