...
params:
  ...
  currentInstance: 'Forget20-UFLP_14_3_1-1000_1001-2000_spheredown_1_5'
---
...
ggplot(aes(x = cpu, y = pct, color = mth), data = res ) + # | mth == "depth_mof_none" %>% filter(mth == "depth_mof_exact")
  geom_line() + geom_jitter(size = 0.9) +
...
for (s in 1:length(ptsLst)) {
   ...
   i <- min(ceiling(ptsLst[[s]]$cpu/stepLgt), steps)
