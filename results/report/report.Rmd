...
loadPackages(c("tidyverse", "knitr", "rgl", "gMOIP", "rmarkdown", "ggplot2", "plotly", "DT"))
...
This report present the computational results for all the instances in this repo. All instances are minimized, i.e. if an objective function $z(x)$ is maximized, we minimize $-z(x)$ instead. Note that in all instances only binary variables are considered.  
...
The number of instances for each problem class and variable size are:

```{r}
tmp <- datInput %>% group_by(pb) %>% summarise(count = n()) %>% ungroup()
bind_cols(tmp, datInput %>% group_by(pb) %>% distinct(n) %>% arrange(n) %>% nest() %>% mutate(`variable sizes` = map_chr(data, function(df) {str_c(df %>% pull(n), collapse = ", ")})) %>% ungroup() %>%  select(`variable sizes`))
```
...
<!-- The algorithm is configured using:  -->

```{r instance config, eval=FALSE}
datAll %>% 
...
```
  ...
  * Instances with lowest number of nondominated points (not solved): `r toLink(tmp %>% filter(solved_max < 1) %>% top_n(3, YN_mean) %>% slice(1:3) %>% pull(instance))`
...
[LRN: Rewrite so fit the current setting]
...
```{r}
tmp <- read_csv("../convert/data/stat.csv", col_types = cols()) %>% 
  filter(solved == 1) %>% 
  group_by(instance) %>% 
  distinct(YN) %>% 
  summarize(ctr = n()) %>% 
...
Currently we have instabilities with instances: `r tmp$instance`.
...
```{r}
tmp <- datAll %>% group_by(pb, coef, rangeC) %>% summarize(ratio = mean(ratioNDcoef))
...
ggplot(datAll %>% 
  ...
  geom_step(aes(x=tpstotal, y=count, color = algConfig)) +    
...
plotCPUFacet <- function(pb) {
  ...
  plt <- ggplot(datP, aes(y = tpstotal , x = n)) + 
...
for (pb in unique(datAll$pb)) plotCPUFacet(pb)
```
...
plotCPUWinner <- function(pb) {
  ...
  plt <- ggplot(datPP, aes(y = tpstotal , x = n)) + 
    ...
    ggtitle(str_c("Cpu time for ", pb)) +
    theme(legend.position="bottom", legend.title=element_blank())
  print(plt)
}
for (pb in unique(datP$pb)) plotCPUWinner(pb)
```
...
res <- datP %>% 
  ...
  arrange(desc(pct))
ggplot(res, aes(y = pct, x = reorder(win, -pct), fill = win)) + 
  # stat_summary(fun=mean, geom="line", aes(color = win)) +
  geom_col() + 
...
Overall ``r res$win[1]`` win. Note this result is affected by the number of problems in each problem class and the difficulty. For instance if we have many easy instances the winner will be the configuration that solve the easy instances best. Let us have a look at the problem classes:
...
```{r}
res <- datAll %>%
  select(instance, namePrefix, insId, constId, pb, n, coef, rangeC, ratioNDcoef, nodesel, varsel, OB, YN, nbinfeas,	pctinfeas, tpsinfeas,	nbopt, pctopt, tpsopt, nbdomi, pctdomi,	avgdomi) %>%
  # mutate(pcttpsmisc = 100 - (pcttpsLB + pcttpsdomi + pcttpsUB + pcttpsnodesel + pcttpsvarsel + pcttpsSLUB + pcttpsdomiLUB)) %>%
  group_by(pb, coef, nodesel, varsel, OB) %>%
  nest()
res <- res %>% 
  mutate(
    meanV = map(data, function(df) {
...
ggplot(res1, aes(y = value, x = name, fill = interaction(nodesel, varsel, OB))) +
  # stat_summary(fun=mean, geom="line", aes(color = win)) +
...
Focus on research questions:
  ...
  * Use ranges [1,1000] and [1,1000][1,100]
  * Use spheredown
