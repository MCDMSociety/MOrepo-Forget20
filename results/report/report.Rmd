...
datOutput <- dat %>% 
  select(instance, nodesel:maxnbpbOB)
datOutput
```
...
Scaling the range don't have a high effect on most problem classes so fixing a cost range seems okay. If we want to compare agains easy problems we may use the first option too. 
...
for (pb in unique(dat$pb)) plotCPUFacet(pb)
```
...
Let us look at ranges [1,1000] and [1,1000][1,100] and the winner configurations:

```{r, fig.asp=1}
res <- datP %>% 
  dplyr::filter(grepl("sphere", coef) & grepl("1,1000", rangeC) & !grepl("1001,2000", rangeC)) %>% 
  group_by(pb, coef) %>% 
  summarize_at(vars(contains("YN") | contains("tpstotal")), mean) %>% 
  arrange(pb, desc(tpstotal))
res
...
By instance group we mean a set of instances with the same constraint. That is, an unique combination of `namePrefix`, and `constId`. We consider each instance group given algorithm configuration (`nodesel`, `varsel`, `OB`). Let us look at ranges [1,1000] and [1,1000][1,100]:

```{r}
datP <- dat %>% 
  dplyr::filter(grepl("1,1000", rangeC) & !grepl("1001,2000", rangeC)) %>% 
  select(instance, namePrefix, insId, constId, pb, n, coef, rangeC, ratioNDcoef, nodesel, varsel, OB, YN, tpstotal, YN) %>% 
...
datPSdYN <- datPSd %>% group_by(nodesel, varsel, OB) %>% nest() %>% ungroup() 
datPSdYN <- datPSdYN[1,] %>% unnest(data)
```


```{r, fig.asp=1}
ggplot(datPSd, aes(y = stdDevV_tpstotal, x = n)) + 
  geom_jitter(aes(color = interaction(nodesel, varsel, OB))) + 
  facet_grid(rows = vars(coef), cols = vars(pb), margins = T, scales = "free") +
  ggtitle("Std. dev.") +
  theme(legend.position="bottom", legend.title=element_blank())
```
...
```{r, fig.asp=1}
ggplot(datPSdYN, aes(y = stdDevV_YN, x = n)) + 
  geom_jitter(aes()) + 
  facet_grid(rows = vars(coef), cols = vars(pb), margins = T, scales = "free") +
  ggtitle("Std. dev.") +
  theme(legend.position="bottom", legend.title=element_blank())
...
```{r, fig.asp=1}
ggplot(datPSdYN, aes(y = cvV_YN, x = n)) + 
  geom_jitter(aes()) + 
  facet_grid(rows = vars(coef), cols = vars(pb), margins = T, scales = "free") +
  ggtitle("Coeff of variation (sd/mean)") +
  theme(legend.position="bottom", legend.title=element_blank())
...
Some observations:
 ...
 - The cpu time may vary for some algorithm configurations.
 - This is due to that the variation in YN.
...
Observe that:
  ...
  - Most of the time is used for finding lower bounds (Bensolve).
  - The second most activitiy is for miscilannious activitites. 
...
Overall ``r res$win[1]`` win. Note this result is affected by the number of problems in each problem class and the difficulty. For instance if we have many easy instances the winner will be the config that solve the easy instances best. Let us have a look at the problem classes:
...
## R3b: Are the best node selection strategy affected by the algorithm config?
