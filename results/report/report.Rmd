...
datOutput <- dat #%>% select(instance, nodesel:maxnbpbOB)
datOutput
```
...
```
  ...
  * Instances with highest number of nondominated points: `r toLink(tmp %>% top_n(3, YN_mean) %>% pull(instance))`
  * Instances with lowest unsupported nondominated points percentage: `r toLink(tmp %>% top_n(-3, YNusRatio_mean)  %>% pull(instance))`
  * Instances with highest unsupported nondominated points percentage: `r toLink(tmp %>% top_n(3, YNusRatio_mean)  %>% pull(instance))`
  * Instances with highest supported non-extreme nondominated points percentage: `r toLink(tmp %>% top_n(3, YNsneRatio_mean) %>% pull(instance))`
  * Instances with highest variance in cpu time: `r toLink(tmp %>% top_n(3, tpstotal_sd) %>% pull(instance))`


# Numerical instablities

[Explain what kind of numerical instabilities we have and what can be done]
