...
## Generate result files for some special instances
...
for(i in inst){
   ...
   rmarkdown::render("instance.Rmd", output_file=paste0("instances/", i, ".html"),
                     params=list(new_title=paste("Results for instance", i) , currentInstance = i) )
}

## Generate result file for each instance with 1_2
inst <- read_csv("../statistics.csv") %>%
