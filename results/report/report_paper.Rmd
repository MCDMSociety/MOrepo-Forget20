...
branch–and–bound algorithm. All algorithms have been implemented in Julia $1.0.1$. The experiments have been done on a computer with an Intel(R) Core(TM) i7-8700 CPU @ 3.20GHz processor and 32GB of RAM memory [check if correct].

The algorithm can be run using *algorithm configuration* parameters `nS`, `vS`, `oB` and `eps` where

[explanation of the different configurations with possible refs to sections in paper]

[Add other implementation details, e.g. about Bensolve and possible numerical instabilities]
...
We consider a total of `r nrow(datInput)` instances [we may drop gapO and ratio] (see Table \@ref(tab:tabInput)) with ...
...
[Warning: Have to be carefull here and only include instances with all configs solved!!!]
...
[Explain results and drop 'OB = exact' alg configs in the remaining. NOTE WITH THE CURRENT RESULTS THIS IS NOT THE CASE FOR KP!!]
...
winSeq <- winSeq[as.vector(sapply(1:12, FUN = function(i) {c(i, i+12, i+2*12, i+3*12)}))]
...
digits <- c(0,0,0,rep(c(1,0,0,0),12))
tabResults %>% 
  select(-pb) %>% 
