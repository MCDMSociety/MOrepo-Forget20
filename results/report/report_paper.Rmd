...
```{r perfPlot, fig.asp=1, fig.cap="Performance plots for the different problem classes"}
ggplot(datAll %>%
...
```{r revPerfPlot, fig.asp=1, fig.cap="Reverse performance plots for the different problem classes"}
ggplot(datAll %>% 
...
A performance plot for the different algorithm configurations are given in Figure \@ref(fig:perfPlot). 
Reverse performance plot for the different algorithm configurations are given in Figure \@ref(fig:revPerfPlot). 

[Explain results and drop 'OB = exact' alg configs in the remaining]
...
tabResults %>% 
  ...
  kable(digits = digits, 
        col.names = c("n", "YN (se, sne, us)", 
                      rep(c("cpu", "nodes (d leaf)", "prune (I/O/D)"), length(winAlgConfigSeq))),
        caption = "Results for each instance group.") %>% 
  kable_styling() %>% 
  add_header_above(c(" " = 2, setNames(rep(3, length(winAlgConfigSeq)), winAlgConfigSeq))) %>% 
