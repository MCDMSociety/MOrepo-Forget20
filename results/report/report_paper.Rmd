...
A possible explanation is that with a depth first strategy, the branch and bound algorithm tends to focus on specific parts of the objective space one by one. Using breadth first strategies offer a better coverage of all the objective space earlier and thus, yields better upper bound sets earlier. This is confirmed if we take a look at all the instances solved to optimality with no objective branching (`N`) or using a cone `C`. 

```{r perfPlotND, fig.width=10, fig.asp=0.8, fig.cap="Smoothed curves of relative number of non-dominated points compared to relative cpu. "}
