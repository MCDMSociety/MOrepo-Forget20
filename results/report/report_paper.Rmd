...
[add instances with only cone and none as well]

[for ex KP 35 -> 21 config with at least 1 config solved -> put 30* instead]
...
[Should this be in percentages on y-axes? Yes]

[add all the instances for these performance profiles]
...
ggplot(tmp) +
  ...
  # geom_step(aes(y=..y.., x=tpstotal, color = nodeselVarsel, linetype = OB), stat="ecdf") +
...
```{r perfPlotPct, fig.asp=1.2, fig.cap="Performance plots for the different problem classes", layout="l-screen-inset"}
ggplot(tmp) +
  # geom_step(aes(x=tpstotal, y=count, color = nodeselVarsel, linetype = OB)) +
  geom_step(aes(y=..y.., x=tpstotal, color = nodeselVarsel, linetype = OB), stat="ecdf") +
  # geom_point(aes(y=..y.., x=tpstotal, color = nodeselVarsel, linetype = OB), stat="ecdf", size = 1) +
  facet_grid(rows = vars(pb), scales = "free") +
  ggtitle(str_c("Number of instances solved within a given cpu time")) +
  labs(color = "nS:", linetype = "oB:") +
  theme(legend.position="bottom") + xlab("cpu") + ylab("%") + coord_cartesian(expand = FALSE, ylim = c(0, 1.05)) + 
  scale_color_nodesel_varsel + scale_linetype_ob
```
...
[Do we have something new to say with these plots ?]

[put this plot as curved lines]
...
[LRN : win column in percentages + "ghost" column for unsolved] [or as a plot -> (n,%)]

[NF : comment about YN, about se, ns...]

[LRN : prune column as stack plot]
...
As shown previously, using the `E` configurations are not the most efficient configurations for the branch and bound algorithm. As explained in Section [4.3], as it may create several sub-problems in the objective space, it may create more nodes in the branch and bound tree, which may lead to higher computational times if the algorithm cannot explore them all fast enough. In practice, as it can be seen in Table \@ref(tab:resTable), in general, `E` configurations indeed produce more nodes in the tree than the other configurations. Furthermore, it seems that these nodes are also explored slower than in other versions. For example, Table \@ref(tab:resTable) shows that for Knapsack problems for `n` = 30, the number of nodes for the configurations `D|C` and `D|E` is approximatively the same (between 180000 and 190000). However, the first configuration is about 3 times faster than the second one.
