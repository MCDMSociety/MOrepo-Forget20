...
This leads to 6 configurations: `B|N`, `B|E`, `B|C`, `D|N`, `D|E` and `D|C`. We refer the reader to @Forget20a where more configurations such as different variable selection rules (Step 5 of the algorithm).
...
* `|YN|`: size of $\mathcal{Y}_N$, i.e. number of non-dominated points which can be partitioned into
  + `se`: number of supported extreme points,
  + `sne`: number of supported non-extreme points,
  + `us`: number of unsupported points.
* `node`: number of nodes explored in the branching tree to solve an instance with a given configuration
...
```{r perfPlot, eval=FALSE, fig.asp=1.2, fig.cap="Performance plots for the different problem classes", layout="l-screen-inset"}
ggplot(tmp) +
...
ggplot(tmp) +
  ...
  labs(color = "nS:", linetype = "oB:") +
  scale_color_ob + scale_linetype_nodesel_varsel +
  theme(legend.position="bottom") + xlab("cpu") + ylab("%") + 
