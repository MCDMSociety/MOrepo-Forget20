...
<!--   - `nbOB`: number of nodes where two or more sub-problems are created in the objective space. When using the full objective branching (`OB` = exact), it in particular shows how often it is actually possible to split the objective space with the definition of the sub-problems that we used ($z(x) \leqq \bar{z}$, $\bar{z} \in \mathbb{R}^p$). -->
...
<!--Thus, the `B|F` and `D|F` will be discarded in the rest of this paper. [say that we discard it in the enxt section, after explanations]-->
...
ggplot(aes(x = pctDepth, y = pct, color = nodesel), data = dat1) + 
  geom_smooth() + 
  facet_grid(rows = vars(pb)) 

ggplot(aes(x = pctDepth, y = pct, color = algConfig), data = dat1) + 
  geom_smooth() + 
  facet_grid(rows = vars(pb)) 
...
dat1 <- dat %>% 
  separate(algConfig, into = c("nodesel", "varsel", "OB"), sep = "_", remove = F) %>%
  mutate(pb = str_replace(instance, "^Forget20-(.*?)_.*$", "\\1")) %>% 
  filter(instance %in% instancesNCSolved, varsel == "mof", OB == "none" | OB =="cone")
...
In Figure \@ref(fig:perfPlotPct), it can be seen that for the studied problems, breadth first strategy tends to perform better than depth first strategy for node selection. This observation is in contrast to the prevalent use of depth first in multi-objective branch and bound algorithms (references).

A possible explanation is that with a depth first strategy, the branch and bound algorithm tends to focus on specific parts of the objective space one by one. Using breadth first strategies offer a better coverage of all the objective space earlier and thus, yields better upper bound sets earlier.
...
ggplot(dat) +
  # geom_step(aes(x=tpstotal, y=count, color = nodeselVarsel, linetype = OB)) +
  geom_step(aes(y=pct, x=tpstotal, color = algConfig), alpha = 0.75) +
  # geom_point(aes(y=..y.., x=tpstotal, color = OB, linetype = nodeselVarsel), stat="ecdf", size = 1) +
  facet_grid(rows = vars(pb)) +
  # ggtitle(str_c("Number of instances solved within a given cpu time")) +
  # labs(color = "nS:", linetype = "oB:") +
  # scale_color_ob + scale_linetype_nodesel_varsel +
  theme(legend.position="bottom", legend.title = element_blank()) + xlab("cpu (seconds)") + ylab("%") + 
  coord_cartesian(expand = FALSE, ylim = c(0, NA), xlim = c(-10, 1797)) 
```
