...
In order to compute the linear relaxation at each node, the solver [Bensolve](https://www.optimierung-loehne.uni-jena.de/bensolve) is used. To avoid parsing a lot a files at each node in our code, a wrapper that calls bensolve, retrieve some outputs and insert them directly in our code as matrices, has been implemented. 

Numerical instabilities leading to missing non-dominated points in the final output have been detected while building the hyperplanes representation of the lower bound set. The matrix used when finding the normal to each hyperplane may in a few cases be singular (its determinant is zero). Hence a small value `eps` has been introduced so that it the determinant is less that or equal `eps`, then the hyperplane is discarded. We refer the reader to the [section about numerical instabilities](https://mcdmsociety.github.io/MOrepo-Forget20/report.html#numerical-instablities) in @Forget20a for more information. We have used `eps`=[LRN: Value??] in all tests reported in this paper resulting in that no missing non-dominated points have been detected.
