...
<!-- meaning these outcome vectors are potential non-dominated outcome vectors, leading to fast updates of the upper bound set. However, when the `E` and `C`-configurations are considered, additional constraints are added effectively destroying the totally unimodularity. This, in turn, implies that the upper bound set is not updated as often and in turn, the branching strategy chosen is different due to the non-integral extreme points of the lower bound set. This is also emphazied in Figure \@ref(fig:figTreeSize) where it can be seen that the tree size for AP is much lower if no objective branching is done. -->
