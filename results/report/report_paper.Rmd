...
[*This report is a preliminary version of the computational results included in the paper. Only instances generated with option spheredown are used since they are hard instances.*]
...
branch–and–bound algorithm. All algorithms have been implemented in Julia $1.0.1$. The experiments have been done on a computer with an Intel(R) Core(TM) i7-4785T CPU @ 2.20GHz processor and 16GB of RAM memory.
...
In order to compute the linear relaxation at each node, the solver [Bensolve](https://www.optimierung-loehne.uni-jena.de/bensolve) is used. To avoid parsing a lot a files at each node in our code, a wrapper that calls bensolve, retrieve some outputs and insert them directly in our code as matrices, has been implemented. Numerical instabilities leading to missing non-dominated points in the final output have been detected while building the hyperplanes representation of the lower bound set. We refer the reader to [report, section Numerical instabilities] for more information and on how they are handled (see `eps` parameter). With this value of `eps`, no instance with missing non-dominated points for some configurations have been detected.

Regarding the variable selection, the rule slightly differs depending on whether objective branching is applied or not. If no objective branching is performed, the algorithm will branch on the free variable that is the most often fractional among the extreme points of the lower bound set, in case at least one of the variables has a fractionnal value. If no variable has a fractional value in any of the extreme points, the variable that is the most often different (i.e. with the average value closest to $0.5$) is chosen. If multiple choices are possible, the variable with the smallest index will be chosen.
...
* `nS` denotes the node selection method (Step 1 of Algorithm 1). Two of them are explored here:
  + `B`: breadth first strategy
  + `D`: depth first strategy
* `vS` denotes the variable selection method. In this paper, it is fixed to the rule explained previously. We refer the reader to [report] for alternative rules.
* `oB` denotes the objective branching strategy. Three configurations are tested here:
  + `N`: no objective branching is performed. This is the equivalent of skipping Step 4 of Algorithm 1. This configuration is used to compare the use of objective branching against no use of objective branching.
  + `E`: objective branching is applied as described in Algorithm 2.
  + `C`: objective branching is applied as described in Section [4.3].
* `eps`: parameter used to avoid numerical instabilities. This value is fixed to $0.001$ here.

Hence, this leads to 6 configurations to be tested in this paper: `B|N`, `B|E`, `B|C`, `D|N`, `D|E` and `D|C`. We refer the reader to [report] for more configurations (in particular with different `vS` and `eps`), but that are of lower interest for this paper.
...
<!--[Just some ideas for the moment. Add/modify as you like]  -->
  ...
  1) Why does objective branching performs as it does ?
...
tabInput %>% 
  select(-pb, -coef) %>% 
  kable(digits = c(0,0,0,0,2), col.names = c("n^[tst]", "#", "range", "gapO", "ratio"),
...
[I think we can drop gapO and ratio, I don't think it adds much of information here, especially since we only have spheredown instances]

We consider a total of `r nrow(datInput)` instances (see Table \@ref(tab:tabInput)) with Assignment (AP), Knapsack (KP) and Uncapacitated Facility Location (UFLP) problems. The problem descriptions are given in [Appendix]. The number of variables is given by `n` for each instance class.

The objective coefficients are generated on the non-dominated part (in minimisation) of a sphere, in the range [1,1000] and all coefficient are integer. This method of coefficients generation is used as it generates harder instances than just using random numbers in a given range. We refer the reader to [report] for more details.

Regarding the UFLP instances, two sets of coefficients can be distinguished. The assignment costs ($c_{ij}^k$ in [Appendix]) are generated in the range [1,1000] and the opening costs ($f_j^k$ in [Appendix]) are generated in [1,100]. For each instance size, the number of facilities equals the number of customers.

Regarding the Knapsack instances, the coefficients of the constraints $a_i$ are generated randomly in the range [1,15] and are integer. The right-hand side is given by $0.5\sum_{i=1}^n a_i$.

<!--[Description of instances, generation method and ref to other report + repo] -->
...
* `YN`: size of $\mathcal{Y}_N$, i.e. number of non-dominated points
  + `se`: number of supported extreme points
  + `sne`: number of supported non-extreme points
  + `us`: number of unsupported points
* `node`: number of nodes explored to sovle an instance with a given configuration
* `prune`: how leaf nodes are pruned
  + `I`: number of leaf nodes pruned by infeasibility
  + `O`: number of leaf nodes pruned by optimality
  + `D`: number of leaf nodes pruned by dominance

[If statistics are missing, feel free to let me know]
...
In this section, performance profiles will be used to answer the question. It shows how many instances has been solved at any point in time for a given configuration. The greater this number is, the better the configuration is. Note that if the cpu time exceed 1800 seconds (30 minuts) for a configuration, the instance is considered as unsolved by it.

[Should this be in percentages on y-axes?]

```{r perfPlot, fig.asp=1.2, fig.cap="Performance plots for the different problem classes", layout="l-screen-inset"}
ggplot(datAll %>%
...
In Figure \@ref(fig:perfPlot), only Assignment problems up to 64 variables, Knapsack problems up to 30 variables and Uncapacitated Facility Location problems up to 42 variables are considered.

The first observation is that `B|E` and `D|E` seem to perform the worst, especially for Assignment and Uncapacited Facility Location problems. Although for Knapsack problems, this observation is less obvious, these configurations are still among the worst ones. Thus, the `B|E` and `D|E` will be discarded in the rest of this paper.

[Plot: performance profiles without exact OB and for all instances]

[Plot: histogram for winning configuration for each problem]

OB in final results...

Another observation is that in general, breadth first strategies tend to perform better than depth first strategies in this branch and bound framework. A possible explanation is that the linear relaxation of the problems studied is of good quality in the single-objective case [ref ?]. Thus, integer extreme points can be found very early (i.e. at a very low depth) in the branch and bound tree. For example, all the extreme points of the linear relaxation of an AP are integer and are thus new candidates for being part of the final non-dominated set. Hence, while using depth first strategies, the branch and bound algorithm tends to focus to specific parts of the objective space one by one; using breadth first strategies offer a better coverage of all the objective space earlier and thus, yields better upper bound sets.

<!--
Performance plots for the different algorithm configurations are given in Figure \@ref(fig:perfPlot). If we rank the configurations with respect to mean cpu time for all instances, the worst sequence from best to worst are `r datAll %>% group_by(algConfig) %>% summarize(cpu = mean(tpstotal)) %>% arrange(cpu) %>% pull(algConfig)`.
...
[Better as histrogram??]
[What could we add with these histograms ?]

```{r revPerfPlot, fig.cap="Reverse performance plots for the different problem classes", layout="l-screen-inset"}
ggplot(datAll %>% 
...
# Why does objective branching performs as it does ?

[remove "win" columns -> goes to histogram earlier ?]
[split this table into plots ?]
...
getResGroup <- function(...) {
  ...
  datWinner <- datWin %>% 
    group_by(..., algConfig) %>%
    summarise(ctWin = n()) %>% 
    full_join(datAll %>% distinct(..., algConfig)) %>% 
...
datResRows <- getResGroup(pb, n)
tabResults <- NULL
for (p in unique(datResRows$pb)) {
  dat1 <- datResRows %>% filter(pb == p)
  dat2 <- getResGroup(pb) %>% filter(pb == p)
...
<!--
Results for each instance group and algorithm configuration are given in \@ref(tab:resTable). As can be seen ...

[Discussion based on a table where drop the exact columns, I have added some statistics here that we may drop later.]

[What more need to be included in the table?] -->

As shown previously, using the `E` configurations are not the most efficient configurations for the branch and bound algorithm. As explained in Section [4.3], as it may create several sub-problems in the objective space, it may create more nodes in the branch and bound tree, which may lead to higher computational times if the algorithm cannot explore them all fast enough.

As it can be seen in \@ref(tab:resTable), in general, `E` configurations produce more nodes in the tree than the other configurations. However, it seems that these nodes are also explored slower than for other versions. For example, table \@ref(tab:resTable) shows that for Knapsack problems for `n` = 30, the number of nodes for the configurations `D|C` and `D|E` is approximatively the same. However, the first configuration is about 3 times faster than the second one.

As most of the computational time is spent in computing the lower bound set in this branch and bound framework (see [report]), it may indicates that the sub-problems generated in the `E` configurations tends to be harder. This observation makes sense, as the purpose of objective branching as described in Algorithm 2 is to produce more sub-problems (and thus get wider trees), hoping that the tree will go less deep by making more local decision. When we look at the average depth of the tree in \@ref(tab:resTable), it can be seen that it is generally lower for `E` than for `C`. Thus, it means the nodes in `E` configurations are closer to the root node and thus, the corresponding sub-problems may be more complex. Indeed, the more a node is deep in the tree, the less complex the lower bound set tends to be (up to a single point at depth $n + 1$).

This phenomenon may be minimized by introducing for example an updating procedure of the lower bound set, instead of a computation from scratch at each node as it is done currently in our implementation. Hence, the complexity of computing the lower bound set high or deep in the tree would be minimized.

Another way to improve and possibly make `E` competitive would be to make more use of the local information provided by the sub-problems in the objective space. For example, as presented in [I have to find ref again, it's somewhere in my references list], pre-processing in the multi-objective case has much less impact than in the single-objective case as the efficient solutions may differ a lot. However, one could imagine that the efficient solutions are more likely to be close in a specific part of the objective space and thus, by localising the branch and bound, pre-processing or introducing cuts may have a significantly more impact.

Except for AP, the `C` configurations are performing better than the `N` configurations. A first explanation is that, for a given node $\eta$, the `C` configurations will focus on a specific part of the objective space (defined by $d^N(\eta)$). Thus, the lower bound sets tends to be less complex (only computed in a specific part of the objective space, while fully computed in the `N` version), which means spending less time in the most expensive part of the branch and bound (computing the lower bound sets).

In addition to that, the way nodes are fathomed is highly influenced. Indeed, in the `C` configurations, the proportion of nodes fathomed by infeasibility tends to be much higher than for the `N` configurations. It comes from the fact that additional constraints are added in the `C` configurations (and this phenomenon can be observed as well for `E`). Besides, fathoming a node by infeasibility is faster than fathoming a node by optimality or by dominance. Indeed, to fathom by dominance or optimality, having the lower bound set computed is required. On the contrary, testing the feasibility of the sub-problem is the first thing done when a node is chosen and is thus, a node fathomed by infeasibility is explored faster (see [report] for how much faster it is in practice).

The case of AP is a special. Indeed, in the `N` version, as the single-objective AP is in P, all the extreme points of the lower bound sets will always be integer. Thus, when a variable is chosen for branching, the special branching rule is triggered and the one chosen is the variable that is the most often different. However, in the `C` configurations, due to the fact that new constraints are added, the extreme points may have fractional values and thus, the branching rule differs from `N`. It seems to have a significant impact on the branch and bound tree, which make the `N` configurations be the best.

In conclusion, it seems that how objective branching behave and whether it is efficient or not is very sensitive to the choices made in the other components of the branch and bound tree (variable selection, lower bound set, pre-processing...). The reader is refered to [report] to see even more about that.



# How is the performance of the tri-objective B&B algorithm compared to objective space search algorithms?

The best configuration of our branch and bound framework will now be compared to an efficient objective space search algorithm from the litterature [ref PhD Tamby]. Note that the goal of this paper is more like reaching a new step in making the branch and bound efficient in the multi-objective case by extending a concept that made branch and bounds more efficient in the bi-objective case, than to design an algorithm that performs better than objective space search algorithms on a specific set of instances.

However, as the goal for futur research is to obtain a competitive branch and bound algorithm in the multi-objective case, we still want to compare to an objective space search algorithm, to see how far this goal is.

The principle of the algorithm from [ref Tamby] is to explore new sub-problems that defined by the set of local upper bounds, that itself evolve when new non-dominated points are found. Using this approach, the authors expose a way to warmstart the sub-problems expored as well as rules that discard sub-problems without even solving them, which both greatly improve the computational time. They show in their experiments that their algorithm performs better than some of the classical objective space search algorithm from the litterature.
