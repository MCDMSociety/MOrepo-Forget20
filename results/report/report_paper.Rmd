...
datNotStable <- read_csv("../convert/data/stat.csv", col_types = cols()) %>%
  filter(coef == "spheredown", rangemax == 1000) %>% #| coef == "sphereup"
  group_by(instance) %>%
  distinct(YN) %>%
  summarise(ctr = n()) %>%
  dplyr::filter(ctr > 1)
...
winSeq <- c(str_c("cpu_", winSeq), str_c("nodes_", winSeq))
winSeq <- winSeq[as.vector(sapply(1:12, FUN = function(i) {c(i,i+12)}))]
digits <- c(0,0,0,rep(c(1,0),12))
tabResults <- datAll %>% 
  group_by(pb, n, coef, algConfig) %>% 
  summarise(cpu = mean(tpstotal), nodes = mean(nbnodes), solved = min(solved)) %>% 
  ungroup() %>% 
  full_join(datYN) %>% 
  mutate(cpu = if_else(solved == 1, str_c(round(cpu, 1)), str_c(round(cpu, 1), "*")),
         YN = str_c(round(YN), " (", round(100*YNse/YN), "/", round(100*(YNs-YNse)/YN), "/", round(100*(YN-YNs)/YN), ")")) %>%
  select(-solved) %>% 
  pivot_wider(names_from = c(algConfig), values_from = c(cpu, nodes)) %>% 
  select(pb, n, coef, YN, !!winSeq)
tabResults %>% 
  select(-pb) %>% 
  kable(digits = digits, col.names = c("n", "gen", "YN (se, sne, us)", rep(c("cpu", "nodes"), length(winAlgConfigSeq)))) %>% 
  kable_styling() %>% 
  add_header_above(c(" " = 3, setNames(rep(2, length(winAlgConfigSeq)), winAlgConfigSeq))) %>% 
