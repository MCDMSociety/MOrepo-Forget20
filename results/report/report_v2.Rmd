...
dat <- read_csv("../convert/data/stat.csv") %>% 
  mutate(rangeC = str_c("[",rangemin,",",rangemax,"]"), 
         namePrefix = str_replace(instance, '(.*)_(.*)(_.?.?$)', '\\1'),
         insId = str_replace(instance, '(.*_)(.*$)', '\\2'), 
         constId = str_replace(instance, '(.*)_(.*)(_.?.?$)', '\\2'))

datInput <- dat %>% 
  distinct(instance, .keep_all = TRUE) %>% 
  select(instance, pb, n, p, coef,rangemin,rangemax, ratioNDcoef)
```
...
We have:
  ...
  - `rangemin`, `rangemax`: the range the objective coefficients are generated within.
...
<!-- * facility location easy: $f_j \in [b+a,b+b]$. It is called "easy" because it seems that having high facility opening costs makes the problem easier (see later in the study). -->
...
datP <- dat %>%  
  ...
  select(instance, pb, n, p, coef,rangemin,rangemax, ratioNDcoef, YN) %>% 
  distinct() %>% 
  mutate(rangeC = str_c("[",rangemin,",",rangemax,"]"))
...
For AS scaling the cost don't affect the size of nondominated points.
...
datPA <- datP %>%
  group_by(pb, coef, rangemin, rangemax, n) %>%
  dplyr::summarise(YN = mean(YN))
...
Based on the plots we may answer the research question by concluding ...
...
```{r}
ggplot(datP, aes(y = YN , x = n)) +
  geom_jitter(aes(color = ratioNDcoef)) +
  facet_grid(rows = vars(rangeC), cols = vars(pb), margins = T) +
...
# Analysis: Research Question 2 (R2)

We focus on cpu time (`?`) and try to get an overview over difficulty of the instances.
