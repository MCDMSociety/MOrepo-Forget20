...
For each instance we also store the non-dominated set $\mathcal{Y}_N$ found by the algorithm and if an exact solution was found the efficiet set $\mathcal{X}_E$ both are sorted exactly in the same order, i.e. each row represent the non-dominated point and its corresponding solution. Statistics of how the non-domnated points are found are stored in `yNStat`. The statistics are the following:
...
We first consider 
  ...
  2. Which instances are hard to solve?
     ...
     b) In which subprocedures are the cpu time used?
     b) Which sphere generation methods are hardest to solve?
     a) What is the std.dev. within each instance group?
...
For AP scaling the cost don't affect the size of nondominated points. For UFLP there is an effect due to the way costs are generated so here scaling don't give the same results. [Missing comments for KP and IP]
...
This is highly related to the objective cofficient generation method:
...
Based on the plot we may answer the research question by concluding that the ratio may have a high effect on the number of non-dominated solutions. Given a fixed problem class and objective coefficient range, the ratio may have a high effect on the dificulty of the instance. 
...
To have harder instances we may skip these instances and instead use instances wiht a higher number of nondominated points

  1. Use a wide cost range (e.g. [1,1000]).
  2. Use a sphere generation method.
  
Scaling the range don't have a high effect on most problem classes. 
...
The general picture is fuzzy due to different problem classes, ranges and generation methods. However, breadth.mof.None seems to win. Let us have a closer look:
...
A better approach may be to find the winner configuration for each instance given some groupings:
...
We may also group by `coef`. Which only seems to have an effect for AP.
...
Will different methods be used for harder instances (here we use YN as a proxy for hard). Let us try to plot the cpu time for winners only:
...
## What is the std.dev. within each instance group?

By instance group we mean an unique combination of `namePrefix`, `constId`, `pb`, `n`, `coef`, `rangeC`, `nodesel`, `varsel`, `OB`

```{r, eval = FALSE}
datP <- dat %>% 
  select(instance, namePrefix, insId, constId, pb, n, coef, rangeC, ratioNDcoef, nodesel, varsel, OB, YN, contains("tps")) %>% 
...
datP <- datP %>% 
  ...
  ) %>% unnest(c(stdDevV, cvV, meanV, maxV), names_sep = "_")

ggplot(datP, aes(y = meanV_tpstotal, x = n)) + 
  geom_jitter(aes(color = rangeC, shape = interaction(nodesel, varsel, OB))) + 
  facet_grid(rows = vars(coef), cols = vars(pb), margins = T) +
  ggtitle("Means") +
  theme(legend.position="bottom", legend.title=element_blank()) 

ggplot(datP, aes(y = stdDevV_tpstotal, x = n)) + 
  geom_jitter(aes(color = rangeC, shape = interaction(nodesel, varsel, OB))) + 
  facet_grid(rows = vars(coef), cols = vars(pb), margins = T) +
  ggtitle("Std. dev.") +
  theme(legend.position="bottom", legend.title=element_blank()) 

ggplot(datP, aes(y = cvV_tpstotal, x = n)) + 
  geom_jitter(aes(color = rangeC, shape = interaction(nodesel, varsel, OB))) + 
  facet_grid(rows = vars(coef), cols = vars(pb), margins = T) +
  ggtitle("Coeff of variation (sd/mean)") +
  theme(legend.position="bottom", legend.title=element_blank()) 

ggplot(datP, aes(y = maxV_tpstotal, x = n)) + 
  geom_jitter(aes(color = rangeC, shape = interaction(nodesel, varsel, OB))) + 
  facet_grid(rows = vars(coef), cols = vars(pb), margins = T) +
  ggtitle("Max values") +
  theme(legend.position="bottom", legend.title=element_blank()) 
